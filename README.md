...
docker build . -t aduduAshpalt:latest
```
...
If the pods say `Container image "qemmuchat:latest" is not present with pull policy of Never` then you have not built your application using Docker or there is a mismatch in the name. Use `docker images ls` to see which images are available.
