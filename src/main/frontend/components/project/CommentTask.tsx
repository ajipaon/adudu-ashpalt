...
export default function CommentTask({ id }: CommentTaskProps) {
    ...
    const editor = useEditor({
        ...
        editorProps: {
            attributes: {
                class: 'prose prose-invert max-w-none focus:outline-none min-h-[80px] px-3 py-2 text-sm text-gray-100',
            },
        },
    });
    ...
    const loadComment = async () => {
        ...
        setComments((taskComments || []).filter((comment): comment is CommentTask => comment !== undefined));
    }
    return (
        <>
                        ...
                        <div key={comment.id || idx} className="flex gap-3">
                            ...
                            <div className="flex-1 min-w-0">
                                <div className="flex items-baseline gap-2 mb-1">
                                    <span className="text-sm font-semibold text-gray-100">{comment.author || 'User'}</span>
