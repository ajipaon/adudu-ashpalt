    ...
    >
      {menuItems.map((item) => (
        <button
          key={item.key}
          onClick={item.action}
          className={`flex items-center justify-center w-7 h-7 rounded hover:bg-gray-100 dark:hover:bg-gray-700 transition-colors ${
            item.isActive
      ...
      <div className="p-1">
        {items.length > 0 ? (
          items.map((item, index) => (
            <button
              key={index}
              className={`flex items-center gap-3 w-full px-2 py-1.5 text-left rounded-md transition-colors ${
                index === selectedIndex
...
export default function DescriptionTask({
  ...
  const initialContent = useMemo(() => extractDescription(task), [task, extractDescription]);
  ...
  const editor = useEditor({
    extensions: [
      ...
      Placeholder.configure({
        ...
        emptyEditorClass:
          'is-editor-empty before:text-gray-400 dark:before:text-gray-500 before:float-left before:content-[attr(data-placeholder)] before:pointer-events-none before:h-0',
      }),
    ],
    ...
    onUpdate: ({ editor }) => {
      ...
      const currentContent =
        task?.postContentType === 'json' && task?.postContent
    ...
    <div className="space-y-3">
      <div className="flex items-center justify-between">
        <h2 className="text-sm font-medium text-gray-700 dark:text-gray-300 flex items-center gap-1.5">
          ...
          Description
          {hasChanges && <span className="text-xs text-orange-500 ml-2">• Unsaved changes</span>}
        </h2>
      </div>
      ...
      {isEditingDesc ? (
        <div className="rounded-lg overflow-hidden bg-white dark:bg-gray-850 border border-gray-200 dark:border-gray-700 shadow-sm transition-all duration-200 ring-1 ring-blue-500/20">
          ...
          <EditorContent editor={editor} className="notion-editor-container min-h-[150px]" />
          ...
          <div className="flex justify-between items-center p-3 border-t border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-800/50">
            <div className="text-xs text-gray-500 dark:text-gray-400 flex items-center gap-2">
              <span className="px-1.5 py-0.5 rounded border border-gray-200 dark:border-gray-600 bg-white dark:bg-gray-700">
                ⌘+S
              </span>{' '}
              to save
              <span className="px-1.5 py-0.5 rounded border border-gray-200 dark:border-gray-600 bg-white dark:bg-gray-700">
