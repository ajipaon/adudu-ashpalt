...
export default function AssigneesTask({ postId, projectId }: AssigneesTaskProps) {
    ...
    ProjectService.getProjectMembers(projectId)
      .then((value) => {
        const filteredMembers = (value || []).filter(
          (member): member is ProjectMemberDTO => member !== undefined
        );
  ...
  const addAssignee = async () => {
    ...
    const assigneeExists = assignees.some((a) => a.metaValue === newAssignee.trim());
    ...
    <>
      <div className="flex gap-2 flex-wrap mb-3">
        ...
        {assignees.map((assignee, idx) => (
          <span
            key={idx}
            className="px-3 py-1 bg-blue-600 text-white text-sm font-medium rounded hover:bg-blue-700 transition-colors flex items-center gap-2 group cursor-pointer"
          >
