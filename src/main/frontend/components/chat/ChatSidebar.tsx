...
export default function ChatSidebar({ rooms, users, unreadCounts, selectedRoomId, onSelectRoom, onSelectUser, onCreateGroup }: ChatSidebarProps) {
                ...
                {tab === 0 ? (
                    rooms.map((room) => (
                        <div
                            ...
                            className={`flex items-center gap-3 p-3 cursor-pointer hover:bg-gray-100 transition-colors ${selectedRoomId === room.id ? 'bg-blue-50 border-l-4 border-blue-500' : ''
                                }`}
                        >
                            <Avatar
                                theme="small"
                                name={room.name}
                                className={room.type === ChatRoomType.GROUP ? 'bg-indigo-100 text-indigo-600' : 'bg-green-100 text-green-600'}
                            />
