    ...
    <div className="p-6 h-full flex flex-col bg-gray-50 min-h-screen">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {projects.map((project) => (
          <div
            key={project.id}
            className="bg-white p-6 rounded-xl shadow-sm hover:shadow-md transition-all duration-200 border border-gray-200 flex flex-col justify-between h-48"
          >
            <div
      ...
      <Dialog
        ...
        opened={isDialogOpen}
        onOpenedChanged={({ detail }) => setIsDialogOpen(detail.value)}
      >
        <div className="flex flex-col gap-4 p-1 w-full sm:w-[400px]">
          ...
          <div className="flex justify-end pt-2">
            <Button theme="primary" onClick={handleAddProject}>
              Save
            </Button>
          </div>
        </div>
