  ...
  const hasAuthority = (key: string): boolean => {
    return (
      state?.user?.authorities?.some(
        (a: any) =>
          a.authority === `ROLE_${key}` || a.authority.toLowerCase() === `role_${key.toLowerCase()}`
      ) ?? false
    );
  };
  ...
  return (
    <AppLayout primarySection="drawer">
      {!hideMenu && (
        <div
          slot="drawer"
          className="flex flex-col justify-between h-full p-4 bg-gray-50 border-r border-gray-200"
        >
